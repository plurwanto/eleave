...
@section('title','Eleave | Claim List')
...
<div class="portlet box green">
    <div class="portlet-title">
        <div class="caption">
            <i class="fa fa-dollar"></i>Claim List
        </div>
        <div class="tools">      
                ...
                <div class="col-md-12">
                    <div class="dt-buttons">
                        <a class="btn btn-default btn-circle btn-sm" tabindex="0" aria-controls="sample_2"
                           href="{{ URL::to('eleave/claim/add') }}"><i class="fa fa-plus"></i> Add Claim</a>
                    </div>
                </div>
            </div>
            ...
            <table class="table table-striped table-bordered table-condensed nowrap" width="100%" id="usertable">
                <thead>
                    <tr>
                        ...
                        <th style="width: 180px">Subject<br />
                            <div class="filter-wrapper"></div>
                        </th>
                        <th>Claim Total</th>
                        <th>Bank Slip</th>
                        <th>Status<br />
                            <div class="filter-wrapper-status"></div></th>
                        <th>Action</th>
                    </tr>
                </thead>
                <tbody>
    ...
    $(document).ready(function () {
        $('#usertable thead th').each(function () {
            ...
            $(this).find(".filter-wrapper-status").html(
                    '<select class="filter form-control">\n\
                        <option value="">- Choose -</option>\n\
                        <option value="{{ session("id") }}">Waiting your revision</option>\n\
        ...
        if ($('#notif').val() == "notif") {
            var url = `${webUrl}eleave/claim/getdataNotif`;
        } else {
            var url = `${webUrl}eleave/claim/getdata`;
        }
...
<script>
    ...
    $(document).on('click', '.reject', function () {
        if (confirm("Are you sure delete ?  ")) {
            ...
            $.ajax({
                type: 'POST',
                url: `${webUrl}eleave/claim/delete`,
                dataType: 'json',
                headers: {
