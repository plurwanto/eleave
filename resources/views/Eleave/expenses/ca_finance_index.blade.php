...
@section('title','Eleave | Cash Advance Finance Approval')
...
<div class="portlet box green">
    ...
    <div class="portlet-body">
        <ul class="nav nav-tabs">
            <li class="active">
                <a href="{{url('eleave/cash_advanceApproval/all_request')}}" id="tab1" aria-expanded="true"> Request <span id="amount_req" class=""> </span></a>
            </li>
            <li class="">
                <a href="{{url('eleave/cash_advanceApproval/realization?status=in_process')}}" id="tab2" aria-expanded="false"> Realization <span id="amount_real" class=""> </span></a>
            </li>
            <li class="">
                <a href="{{url('eleave/cash_advanceApproval/realization?status=done')}}" id="tab3" aria-expanded="false"> History </a>
            </li>
        </ul>
                    ...
                    <table class="table table-striped table-bordered table-condensed nowrap" width="100%" id="usertable">
                        <thead>
                            <tr>
                                ...
                                <th style="width: 115px">Number<br />
                    ...
                    <div class="form-body">
                        ...
                        <div class="fin1">
                            <div class="form-group">
                                ...
                                <div class="col-md-9">
                                    <input type="hidden" value="" name="voucher_no_temp" id="voucher_no_temp">
...
//        } else {
        var url = `${webUrl}eleave/cash_advanceApproval/get_all_data`;
        //  }
...
//        });
        ...
        form1.validate({
            ...
            messages: {
                ...
                'voucher_no': {
                    required: function () {
                        toastr.error('voucher no is required')
                    },
                },
                'voucher_file': {
                    required: function () {
                        toastr.error('voucher slip is required')
                    },
                ...
                'cheque_no': {
                    required: function () {
                        toastr.error('cheq no is required')
                    },
                },
                'cheq_file': {
                    required: function () {
                        toastr.error('cheq slip is required')
                    },
                    extension: "Invalid file type",
                    filesize: "less than 2MB"
                }
    ...
    function process_this(id, ca_id, ca_approver1) {
        save_method = 'update';
        $('#form')[0].reset();
        ...
        $.ajax({
            ...
            success: function (response) {
                $('#ca_id').val(response.data[0].ca_id);
                $('#ca_id_show').val(response.data[0].ca_id);
                $('#user_id').val(id);
                $('#subject').val(response.data[0].ca_subject);
                $('#user_name').val(response.data[0].ca_username);
                $('#voucher_no_temp').val(response.data[0].ca_voucher);
                $('#voucher_no').val(response.data[0].ca_voucher);
                $('#cheque_no').val(response.data[0].ca_cheque);
                $('#img_voucher').attr("src", response.data[0].ca_voucher_slip);
                $('#img_cheque').attr("src", response.data[0].ca_bank_slip);

                if (id_eleave == ca_approver1) {
                    $('.fin2').hide();
                    $('span.fin1_image').show();
                    $('input', '#voucher_no').removeClass('disabled');
                } else {
                    $('.fin2').show();
                    $('span.fin1_image').hide();
                    $('input', '#voucher_no').addClass('disabled');
                }

            }
        });

        $('.modal-title').text('Processing Form');
    ...
    function process_request() {
        if (!$("#form").valid()) { // Not Valid
            ...
            if (save_method == 'update') {
                url = `${webUrl}eleave/cash_advanceApproval/fa_update_request`;
            }
