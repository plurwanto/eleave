...
@section('title','Eleave | Stationery Add')
...
<div class="portlet box green">
    <div class="portlet-title">
        <div class="caption">
            <i class="fa fa-plus-square"></i>Stationery Add
        </div>
        <div class="tools"></div>
    </div>
    <div class="portlet-body">
        <!-- BEGIN FORM-->
        <form id="form_inventory" name="form_inventory" action="{{url('eleave/inventory/insert')}}" class="form-horizontal" method="post">
            <div class="form-body">
                <div class="col-md-4">
                ...
                <div class="row">
                    <div class="col-md-offset-3 col-md-9">
                        ...
                        <a href="{{ URL::to('eleave/inventory/index') }}"  class="btn btn-circle grey-salsa btn-outline">Back</a>
                        <br>
                        <br>
                    </div>
                </div>
            ...
            if (j < 2) {
                ...
                for (var i = 1; i <= j; i++) {
                    ...
                    if (quantity > inventory) {
                        alert('Your order quantity ' + quantity + ' is greater than ' + inventory + ' stock');
                        $('#quantity_' + i).select();
                        return false;
                    }
                }
...
<script>
    ...
    function tab1_To_tab2(id)
    {
        ...
        cell5.setAttribute("class", "hidden");
        ...
        cell5.innerHTML = "<input type='text' value='" + table1.rows[id].cells[3].innerHTML + "' name='stock[]' id='stock_" + id + "' class='form-control text-right input-sm stk_" + i + "'>";
        cell6.innerHTML = "<input type='text' name='quantity[]' id='quantity_" + id + "' class='form-control changesNo text-right input-xs qty_" + i + "' autocomplete='off' onkeypress='return IsNumeric(event);' min='1' value='1'>";
    ...
    function get_data(url, q) {
        if (!url)
            url = `${webUrl}eleave/inventory/get_filter_name`;
        var arr_id = [];
    ...
    function choose_item(id, nama, qty) {
        var numb = $('#row_number').val();

        $('#itemNo_' + numb).val(id);
        $('#itemName_' + numb).val(nama);
        $('#stock_' + numb).val(qty);
        $('#modal_form').modal('hide');
        $('#quantity_' + numb).focus();

        var lastRow = $('#barangtable tr').length;
        var kodebarang = $('#itemNo_' + numb);
        var namabarang = $('#itemName_' + numb);
        var stockbarang = $('#stock_' + numb);
        var exist = false;
        //alert(lastRow);
        for (a = 1; a < lastRow; a++) {
            if (a == numb) {
