...
<form id="formInputEdit"  class="form form-horizontal">
                        ...
                        <div class="col-md-12 ">
                            <div class="form-body">
                                <div class="form-group">
                                    ...
                                    <div class="col-md-9">
                                        <input type="text" name="name" value="{{$row->result->doc_type_name}}" autocomplete="off" class="form-control" >
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="col-md-3 control-label">Department <font color="red">*</font></label>
                                    <div class="col-md-9">
                                        <select name="departement" class="form-control">
                                            <option value="">-- Choose a Department --</option>
                                            @for($i = 0; $i < count($departement->result); $i++)
                                                @if($departement->result[$i]->doc_dept_id == $row->result->doc_dept_id)
                                                    <option value="{{$departement->result[$i]->doc_dept_id}}" selected>{{$departement->result[$i]->doc_dept_name}}</option>
                                                @else
                                                    <option value="{{$departement->result[$i]->doc_dept_id}}">{{$departement->result[$i]->doc_dept_name}}</option>
                                                @endif
                                            @endfor
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-md-9">
                                        <div class="mt-radio-list">
                                            <label class="mt-radio mt-radio-outline">
                                                @if($row->result->doc_type_active =='Y')
    ...
    <input type="hidden" name="id" value="{{$row->result->doc_type_id}}">
