                            ...
                            <form id="formInputExtend"  class="form form-horizontal">
                                <div class="row">
                                    <div class="col-md-12"  style="margin-bottom: 20px">
                                            ...
                                            <div class="col-md-4" style="padding: 0px 30px;">
                                                <div class="form-group form-margin-0">
                                                    <div class="validate">
                                                        ...
                                                        <select id="status" class="search-input-select form-control input-sm" disabled  onchange="statusContract()">
                                            ...
                                            <div class="col-md-4" style="padding: 0px 30px;">
                                                <div class="form-group form-margin-0">
                                                    ...
                                                    <div style="clear: both; margin-bottom: 10px;"></div>
                                                    <div class="validate">
                                                        <label style="font-weight: bold;">With Memo</label>
                                                        <select name="memo" id="memo" class="search-input-select form-control input-sm" onChange="setMemo()">
                                            ...
                                            <div class="col-md-12" style="border-top: 1px solid #444d58; margin-bottom: 20px;display:none" id="memoArea">
                                                <table class="table table-bordered table-hover" width="100%" style="margin-top: 20px">
                                                    <thead>
                                                        <tr>
                                                            <th> # </th>
                                                            <th> From </th>
                                                    ...
                                                    <tbody>
                                                        <tr  class="active validate">
                                                            <td><label><input type="checkbox" checked='checked' disabled> Effective Date</label></td>
                                                            <td style="width:250px; text-align:right"></td>
                                                            <td> <input autocomplete="off" type="text" class="form-control date-picker" name="effective_date" id="effective_date"> </td>
                                                        </tr>
                                                        <tr  class="active validate">
                                                            <td style="width:200px"><label><input type="checkbox" onclick="ckChange()" id="condition1" value="1"> Departement</label></td>
                                                            <td  style="width:250px; text-align:right"> {{ $contract->contract->cont_dept}} </td>
                                                            <td  style="width:250px"><input autocomplete="off" type="text" class="form-control" name="departement" id="departement" disabled> </td>
                                                        </tr>
                                                        <tr  class="active validate">
                                                            <td><label><input type="checkbox" onclick="ckChange()" id="condition3" value="1"> Position Change</label></td>
                                                            <td style="width:250px; text-align:right"> {{ $contract->contract->cont_position}}  </td>
                                                            <td> <input autocomplete="off" type="text" class="form-control" name="position" id="position" disabled> </td>
                                                        </tr>
                                                        <tr  class="validate">
                                                            <td><label><input autocomplete="off" type="checkbox" onclick="ckChange()" id="condition2" value="1"> Salary Increment / Basic Salary</label></td>
                                                            <td style="width:250px; text-align:right">{{ $contract->contract->cur_id}}.
                                                            @php
                                                            if($contract->contract->cur_id =='IDR'){
                                                            echo $contract->contract->cont_basic_salary;
                                                            }else{
                                                            echo $contract->contract->cont_basic_salary;
                                                            }
                                                            @endphp
                                                            ...
                                                            <td>
                                                            <input autocomplete="off" type="text" class="form-control" name="salary" id="salary" onkeypress="formatRupiah()" disabled>
                                                        ...
                                                        <tr  class="active validate">
                                                            ...
                                                            <label>
                                                            <input type="checkbox" name="condition4" onclick="ckChange()" id="condition4" value="1">
                                                                Allowance
                                                            </label>

                                                                <div style="padding: 0px 30px; display:none" class="col-md-12" id="allowance">
                                                                <br>
                                                                    <div class="form-group form-margin-0">
                                                                        <div>
                                    ...
                                    <div class="modal-footer">
                                        <input type="hidden" id="currency" value="{{ $contract->contract->cur_id}}">
                                        <input type="hidden" id="departement_val" autocomplete="off" value="{{ $contract->contract->cont_dept}}">
                                        @if($contract->contract->cur_id =='IDR')
                                        <input type="hidden" id="salary_val" autocomplete="off" value="{{ $contract->contract->cont_basic_salary}}">
                                        @else
                                        <input type="hidden" id="salary_val" autocomplete="off" value="{{ $contract->contract->cont_basic_salary}}">
                                        @endif
                                        <input type="hidden" id="position_val" autocomplete="off" value="{{ $contract->contract->cont_position}}">

                                        <input type="hidden" name="cont_id" autocomplete="off" value="{{ $contract->contract->cont_id}}">
                                        @php
...
<div class="modal fade bs-modal-md portlet" id="alert-notif" role="dialog" aria-hidden="true">
    ...
    <div class="modal-content">
      ...
      <table width="60%">
        ...
        <tr>
            <td style="width:210px">Departement</td><td style="width:15px">:</td><td><span id="departement_mod"></span></td>
        </tr>
        <tr>
            <td>Salary Increment / Basic Salary</td><td>:</td><td><span id="salary_mod"></span></td>
        </tr>
        <tr>
            <td>Position</td><td>:</td><td><span id="position_mod"></span></td>
        </tr>
        <tr>
            <td>Effective Date</td><td>:</td><td><span id="effective_date_mod"></span></td>
        </tr>
        @if($contract->contract->mem_citizenship == 'expatriate')
        <tr>
...
<script type="text/javascript">
        ...
        $("#buttonConfrim").click(function(){
            ...
            $.ajax({
                ...
                url: "{{ URL::asset(env('APP_URL').'/hris/administration/contract/doextend') }}",
                data: data,
                processData: false,
                contentType: false,
                cache: false,
...
function preview(){
    ...
    if(position != undefined && position !=''){
        $('#position_mod').html(position);
    }else{
        $('#position_mod').html($('#position_val').val());
    }

    var salary = $('#salary').val();
    if(salary != undefined && salary !=''){
        $('#salary_mod').html($('#currency').val()+'. '+salary);
    }else{
        $('#salary_mod').html($('#currency').val()+'. '+$('#salary_val').val());
    }

    var departement = $('#departement').val();
    if(departement != undefined && departement !=''){
        $('#departement_mod').html(departement);
    }else{
        $('#departement_mod').html($('#departement_val').val());
    }

    $('#effective_date_mod').html($('#effective_date').val());
    ...
    var allowance = '';
    if(condition4.checked == true && memo.value == 'Y'){

        var allowance = '<h5>Allowance</h5>'
            $.each($('.amount'), function(i,v){
                var allow = $(`.allowance_type option:selected`)[i].text
                if(v.value != undefined &&	v.value != ""){
                    allowance +=`<tr>
                                    <td style="width:200px">${allow}</td><td style="width:15px">:</td><td>${currency.value}. ${v.value}</td>
                                </tr>`
                }
            })
    }
...
function ckChange(){
    ...
    if(memo.value == 'Y'){
        if(condition1.checked == false && condition2.checked == false && condition3.checked == false && condition4.checked == false){
