...
<div class="modal-header">
        @php
            if($period !=0){
                echo '<a href="'. URL::asset(env('APP_URL').'hris/administration/bpjs/kesehatan').'?period='. $period .'" type="button" class="close"></a>';
            }else{
                echo '<a href="'. URL::asset(env('APP_URL').'hris/administration/bpjs/kesehatan') .'" type="button" class="close"></a>';
            }
        @endphp
...
<form id="formInput" action="{{ URL::asset(env('APP_URL').'hris/administration/bpjs/kesehatan') }}" method="get"  class="form form-horizontal">
                                    ...
                                    <div class="col-md-4">
                                        <select class="form-control" name="month">
                                            ...
                                            for ($i = 0; $i < count($month); $i++) {
                                                if ($month_num[$i] == substr($period,5,7)) {
    ...
    <div class="modal-footer">
        @php
            if($period !=0){
                echo '<a href="'. URL::asset(env('APP_URL').'hris/administration/bpjs/kesehatan').'?month='. substr($period,5,7) .'&year='. substr($period,0,4) .'" type="button" class="btn dark btn-outline">Close</a>';
            }else{
