    ...
    <div class="pull-right">
        ...
        <div class="btn-group pull-right">
            ...
            <ul class="dropdown-menu pull-right">
                ...
                @php
                    if($access->menu_acc_add == 1){
                        echo  '<a href="'. URL::asset(env('APP_URL').'/hris/administration/bpjs/ketenagakerjaan/template-iuran') .'"><i class="fa fa-file-excel-o" style="
                margin-right: 3px;
                "></i>Template </a>';
                    }
                @endphp
                        ...
                        <div class="portlet-body">
                                    ...
                                    <div class="col-md-10">
                                        <div class="pull-right">
                                            <div class="pull-left text-left" style="display: inline-flex;">
                                                <select name='cus_id' id="cus_id" data-column="3" class="border-rounded input-sm" style="margin-right: 10px;height: 34px;
                                                        padding: 5px 10px;
                                                        font-size: 12px;
                                                    ...
                                                    <select data-column="4" id="searchMonth" class="border-rounded input-sm" name="month" style="margin-right: 10px;height: 34px;
                                                    padding: 5px 10px;
                                                    font-size: 12px;
                                                    line-height: 1.5;  background-color: #fff;
                                                    border: 1px solid #c2cad8;
                                                    ...
                                                    <input data-column="5" id="searchYear" class="border-rounded input-sm" style="margin-right: 10px; width: 75px;height: 34px;
                                                    padding: 5px 10px;
                                                    font-size: 12px;
                                                    line-height: 1.5;    background-color: #fff;
                                                    border: 1px solid #c2cad8;
                            ...
                            <table class="table table-striped table-bordered table-advance table-hover" id="users-table">
                                ...
                                <th></th>
                                    ...
                                    <th></th>
                                    <th></th>
                                    <th><input type="text" data-column="2"  class="search-input-text form-control input-sm   border-rounded" placeholder="Search User"></th>
                                    <th></th>
                                </tr>
                                </thead>
                            </table>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function () {
                ...
                $('.search-button').on('click', function () {   // for search
                    ...
                    dataTable.columns(3).search(customer).draw();
                    dataTable.columns(4).search(searchMonth).draw();
                    dataTable.columns(5).search(searchYear).draw();
                });
            });
            ...
            function get_modal(e)
            {
                ...
                if (action == 'upload') {
                    ...
                    $.get("{{ URL::asset(env('APP_URL').'/hris/administration/bpjs/ketenagakerjaan/upload') }}",
                    {
                ...
                if (action == 'filterExcel') {
                    $('#modalAction').modal({backdrop: 'static', show: true, keyboard: false});
                    $.get("{{ URL::asset(env('APP_URL').'/hris/administration/bpjs/ketenagakerjaan/filter/excel') }}",
                    {
                        searchMonth: searchMonth,
                        searchYear: searchYear,
                        cus_id: cus_id,
                ...
                if (action == 'detail') {
                    $('#modalAction').modal({backdrop: 'static', show: true, keyboard: false});
                    $.get("{{ URL::asset(env('APP_URL').'/hris/administration/bpjs/ketenagakerjaan/detail') }}",
                    {
                        searchMonth: searchMonth,
                        searchYear: searchYear,
                        id: arr[0]
            ...
            {
                if (elm.value != '') {
                    window.location = "{{ URL::asset(env('APP_URL').'/hris/administration/bpjs/ketenagakerjaan') }}?link="+ elm.value;
                }
            }
        </script>
@endsection
