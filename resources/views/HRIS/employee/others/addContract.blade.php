...
<div style="height: 30px;margin: 0px 0px 15px 0px;">
    ...
    @php
    echo '<a  class="btn dark btn-outline btn-circle btn-md border-rounded" href="'. URL::asset(env('APP_URL').'/hris/employee/others') .'">Back</a>';
    @endphp
    </div>
 </div>
@endsection
                ...
                $.ajax({
                    ...
                    if (data.response_code == '200') {
                        swal({title: "Input Successfully", type: "success"},
                                function () {
                                    window.location = "{{ URL::asset(env('APP_URL').'/hris/employee/others') }}?link="+data.customer;
