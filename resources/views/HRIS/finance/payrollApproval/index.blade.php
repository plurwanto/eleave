        ...
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                var dataTable = $('#users-table').DataTable({
                    ...
                    "order": [[ 0, "DESC" ]],
                    "ajax": {
                        "url": "{{ URL::asset(env('APP_URL').'/hris/finance/payroll/approval/listdata') }}",
                        "dataType": "json",
                        "type": "POST",
                        "data": {"_token": "<?=csrf_token()?>"}
                    },
                    "columns": [
                        { data: 'app_id', name: 'app_id', visible: false},
            ...
            function get_modal(e)
            {
                ...
                if (action == 'upload') {
                    $('#modalAction').modal({backdrop: 'static', show: true, keyboard: false});
                    $.get("{{ URL::asset(env('APP_URL').'/hris/finance/payroll/approval/upload') }}",
                    {
                        link: link,
                ...
                if (action == 'edit') {
                    $('#modalAction').modal({backdrop: 'static', show: true, keyboard: false});
                    $.get("{{ URL::asset(env('APP_URL').'/hris/finance/payroll/approval/edit') }}",
                    {
                        link: link,
                        id: arr[0]
                    },
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        $.get("{{ URL::asset(env('APP_URL').'/hris/finance/payroll/approval/do-delete') }}",
                            {
                                link: link,
                                id: arr[0]
                            },
                            function (data) {
                                ...
                                function () {
                                    window.location = "{{ URL::asset(env('APP_URL').'/hris/administration/letter-request') }}";
