...
<form id="formInputReject"  class="form form-horizontal">
    ...
    <input type="hidden" name="id" value="{{$id}}">
        ...
        <button type="submit" class="btn red" id="btnSubmit">Reject</button>
    </div>
</form>
...
<script type="text/javascript">
        ...
        $(document).on("submit", "#formInputReject", function (event)
        {
            $('.loading').show();
            //stop submit the form, we will post it manually.
            event.preventDefault();
            // Get form
            var form = $('#formInputReject')[0];
            ...
            $.ajax({
                ...
                url: "{{ URL::asset(env('APP_URL').'/hris/finance/payroll/approval/do-reject') }}",
                data: data,
                processData: false,
                contentType: false,
                cache: false,
                ...
                success: function (data) {
                    ...
                    if (data.response_code == '200') {
                        swal({title: "Input Successfully", type: "success"},
                                function () {
                                    window.location = "{{ URL::asset(env('APP_URL').'/hris/finance/payroll/approval') }}?link={{$link}}";
